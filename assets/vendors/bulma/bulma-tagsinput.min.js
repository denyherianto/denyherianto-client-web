;(function(a, b) {
  ...
  class c {
    ...
    init() {
      if (!this.options.disabled) {
        ...
        let a = this.element.getAttribute("type")
        ;(a && "tags" !== a) || (a = "text"),
          (this.input = document.createElement("input")),
          this.input.setAttribute("type", a),
        ...
        let b = this.element.nextSibling
        this.element.parentNode[b ? "insertBefore" : "appendChild"](
          this.container,
          b
        ),
    ...
    enable() {
      this.enabled ||
        ...
        this.input.addEventListener("keydown", a => {
          let c,
            d = a.charCode || a.keyCode || a.which,
            e = this.container.querySelector(".tag.is-active"),
            f = Array.prototype.slice
              .call(this.container.querySelectorAll(".tag:not(.is-delete)"))
              .pop(),
            g = this.caretAtStart(this.input)
          if (
            (e &&
              (c = this.container.querySelector(
                '[data-tag="' + e.innerHTML.trim() + '"]'
              )),
            this.setInputWidth(),
            d === 13 ||
              d === this.options.delimiter.charCodeAt(0) ||
              d === b ||
              d === 9)
          ) {
            if (
              !this.input.value &&
              (d !== this.options.delimiter.charCodeAt(0) || d === b)
            )
              return
            this.savePartial()
          } else if (d === 46 && c)
            c.nextSibling
              ? this.select(c.nextSibling.querySelector(".tag"))
              : c.previousSibling &&
                this.select(c.previousSibling.querySelector(".tag")),
              this.container.removeChild(c),
              delete this.tags[this.tags.indexOf(c.getAttribute("data-tag"))],
              this.setInputWidth(),
              this.save()
          else if (d === 8) {
            if (c)
        ...
        this.container.addEventListener("touchstart", a => {
          this.refocus(a)
        }),
    ...
    select(a) {
      let b = this.container.querySelector(".is-active")
      b && b.classList.remove("is-active"), a && a.classList.add("is-active")
    }
    addTag(b) {
      if (
        (~b.indexOf(this.options.delimiter) &&
          (b = b.split(this.options.delimiter)),
        Array.isArray(b))
      )
        return b.forEach(a => {
          this.addTag(a)
      ...
      if (
        ((this.element.getAttribute("lowercase") ||
          "true" == this.options.lowercase) &&
          (c = c.toLowerCase()),
        (this.element.getAttribute("uppercase") ||
          "true" == this.options.uppercase) &&
          (c = c.toUpperCase()),
        "true" == this.element.getAttribute("duplicates") ||
          this.options.duplicates ||
          -1 === this.tags.indexOf(c))
      ) {
        this.tags.push(c)
        let b = document.createElement("div")
        ;(b.className = "control"), b.setAttribute("data-tag", c)
        let d = document.createElement("div")
        ;(d.className = "tags"), d.classList.add("has-addons")
        ...
        if (
          ...
          ;(b.className = "tag"),
            b.classList.add("is-delete"),
            a.forEach(a => {
              b.addEventListener(a, a => {
                let b,
                  c = a.target.parentNode,
                  d = Array.prototype.slice
                    .call(this.container.querySelectorAll(".tag"))
                    .pop(),
                  e = this.caretAtStart(this.input)
                if (
                  (c &&
                    (b = this.container.querySelector(
                      '[data-tag="' + c.innerText.trim() + '"]'
                    )),
                  b)
                )
                  this.select(b.previousSibling),
                    this.container.removeChild(b),
                    delete this.tags[
                      this.tags.indexOf(b.getAttribute("data-tag"))
                    ],
                    this.setInputWidth(),
                    this.save()
                else if (d && e) this.select(d)
                else return
              })
            }),
            d.appendChild(b)
        }
        b.appendChild(d), this.container.insertBefore(b, this.input)
      }
    }
    getValue() {
    ...
    setValue(a) {
      Array.prototype.slice
        .call(this.container.querySelectorAll(".tag"))
        .forEach(a => {
          delete this.tags[this.tags.indexOf(a.innerHTML)],
    ...
    setInputWidth() {
      ...
      this.container.offsetWidth &&
        (this.input.style.width =
          Math.max(
            this.container.offsetWidth -
              (a ? a.offsetLeft + a.offsetWidth : 30) -
              30,
            this.container.offsetWidth / 4
          ) + "px")
    }
    savePartial(a) {
      "string" == typeof a || Array.isArray(a) || (a = this.input.value),
        !1 !== this.addTag(a) &&
          ((this.input.value = ""), this.save(), this.setInputWidth())
    }
    save() {
    ...
    caretAtStart(a) {
      try {
        return 0 === a.selectionStart && 0 === a.selectionEnd
      } catch (b) {
        return "" === a.value
      }
    }
    refocus(a) {
      return (a.target.classList.contains("tag") && this.select(a.target),
      a.target === this.input)
        ? this.select()
        : (this.input.focus(), a.preventDefault(), !1)
    }
    reset() {
      this.tags = []
    }
  ...
  return (
    document.addEventListener("DOMContentLoaded", function() {
      let a = document.querySelectorAll('input[type="tags"]')
